...
echo "$($PSQL "SELECT DISTINCT(name) from teams FULL JOIN games ON teams.team_id=games.winner_id FULL JOIN games as g ON teams.team_id = g.opponent_id WHERE (g.year=2014 AND g.round='Eighth-Final') OR (games.year=2014 AND games.round='Eighth-Final') ORDER BY name")"
